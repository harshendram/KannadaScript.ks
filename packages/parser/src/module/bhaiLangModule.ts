...
import FunctionCall from "../components/parser/statement/expression/functionCall";
...
import DictionaryLiteral from "../components/parser/statement/expression/literals/dictionaryLiteral";
...
export default class bhaiLangModule {
  ...
  private static _dictionaryLiteral?: DictionaryLiteral;
  ...
  static getDictionaryLiteral() {
    if (!this._dictionaryLiteral) {
      this._dictionaryLiteral = new DictionaryLiteral(this.getTokenExecutor());
    }
    return this._dictionaryLiteral;
  }
  ...
  private static _functionCall?: FunctionCall;
  ...
  static getFunctionDeclaration() {
    if (!this._functionDeclaration) {
      this._functionDeclaration = new FunctionDeclaration(this.getTokenExecutor());
    }
  ...
  static getFunctionCall() {
    if (!this._functionCall) {
      this._functionCall = new FunctionCall(this.getTokenExecutor());
    }
